class FootballPlayer < ApplicationRecord
  ...
  def self.import_argentinian_players
    #Acuna
    FootballPlayer.create(name: "Marcos Acuna", rank: 13, country: "ARG", position: "defender", squad: "Sevilla", league: "La Liga", age: 30, born: 1991, minutes_played: 2260, goals: 105,
    penalty_kicks_made: 705, matches_played: 31,corner_kicks: 248,yellow_card: 81,red_card: 267,penalty_kicks_won: 235,fouls_committed: 237)
    #Alario
    FootballPlayer.create(name: "Lucas Alario", rank: 50, country: "ARG", position: "forward", squad: "Leverkusen", league: "Bundesliga", age: 29, born: 1992, minutes_played: 655, goals: 79,
    penalty_kicks_made: 192, matches_played: 27,corner_kicks: 193,yellow_card: 148,red_card: 263,penalty_kicks_won: 64,fouls_committed: 172)
    #Aguero
    FootballPlayer.create(name: "Sergio Aguero", rank: 27, country: "ARG", position: "forward", squad: "Barcelona", league: "La Liga", age: 33, born: 1988, minutes_played: 151, goals: 179,
    penalty_kicks_made: 402, matches_played: 4,corner_kicks: 52,yellow_card: 74,red_card: 81,penalty_kicks_won: 134,fouls_committed: 146)
  end
  ...
  def self.update_statistics(stat)
    stat.each do |playerstat|
      if FootballPlayer.find_by(name: "#{playerstat[0]}") == nil
        raise ActiveRecord::RecordNotFound, "Player not found #{playerstat[0]}"
      else
        player = FootballPlayer.find_by(name: "#{playerstat[0]}")
        player.goals += playerstat[1]
        player.minutes_played += playerstat[2]
        player.red_card += playerstat[3]
        player.yellow_card += playerstat[4]
        player.save
      end
    end
  end
  ...
  def self.ban(name)
    if FootballPlayer.find_by(name: "#{name}") == nil
      raise ActiveRecord::RecordNotFound, "Player not found #{name}"
    else
      FootballPlayer.destroy_by(name: "#{name}")
    end
  end
end
