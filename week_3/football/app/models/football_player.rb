...
FootballPlayer.create(
  ...
  def self.update_statistics(stat)
    ...
    stat.each do |plr|
      x=FootballPlayer.find_by(name:plr[0])
      if(x.nil?)
        raise ActiveRecord::RecordNotFound,"#{plr[0]} not found"
      end
      x.update(goals:(x.goals)+plr[1],minutes_played:(x.minutes_played)+plr[2],red_card:(x.red_card)+plr[3],yellow_card:(x.yellow_card)+plr[4])
    end
