class Pet
  ...
  def food_required(days)
    self.food_consumed_per_day * days
  end
  ...
  def self.cost_to_feed(pets, days)
    total = 0
    pets.each do |pet|
      total += pet.food_cost_per_day * days
    end
    total
  end
  ...
  def self.group_by_animal_type(pets)
    pets_by_type = {}
    pets.each do |pet|
      if(pets_by_type[pet.animal_type_id] == nil)
        pets_by_type[pet.animal_type_id] = []
      end
        pets_by_type[pet.animal_type_id].push(pet.name)
      end
  pets_by_type
  end
end
