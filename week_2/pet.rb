class Pet
  ...
  def habitat
    HABITATS.each do |key, animal_type|
      if animal_type.include?(animal_type_id)
        return key
      end
    end
  end
  ...
  def food_cost_per_day
    return food_consumed_per_day* FOOD_COST_PER_KG[animal_type_id]
  end
